...
class torchSaab(nn.Module):
	...
	def fit(self, x, bias=None):
		...
		#sample_patches_ = sample_patches[self.grp]
		...
		rank_ac = dim-1
		sample_patches_ = sample_patches
	...
	def inverse_saab(self,x):
		...
		ac_comp = torch.matmul(sample_patches[:, 1:], self.ac_kernels[:c-1])
		...
		ones_mask = torch.ones_like(sample_rec)
		count_mask = F.fold(ones_mask, output_size=(self.in_w, self.in_h), kernel_size=self.kernel_size, stride=self.stride)
		
		return out/count_mask
...
class sslModel(nn.Module):
	def __init__(self, layers, comp_mode):
		super().__init__()
		self.layers = nn.ModuleList(layers)
		self.compute_mode = comp_mode
...
if __name__=="__main__":
	...
	ud_idx = idx.copy()
	rot_idx = idx.copy()
	layer1 = torchSaab(kernel_size=(4,4), stride=(1,1), pad = (0,0), ud_idx= ud_idx, rot_idx = rot_idx, grp = grp, channelwise=False)
