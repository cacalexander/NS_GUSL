...
def grad_mag(im):
    ...
    for i in range(len(im)):
        Ix = sobel(im[i], axis=1)
        Iy = sobel(im[i], axis=0)

        grad_all.append(np.sqrt(Ix**2 + Iy**2)   )

    return np.asarray(grad_all)
...
class Model:
    ...
    def lp(self, X0):
        ...
        #X0 = np.pad(np.squeeze(X0), ((0,0),(2,2),(2,2)), 'reflect')
        #Y = np.pad(Y, ((0,0),(2,2),(2,2)), 'reflect')
        X0 = np.squeeze(X0)
        #Y = np.squeeze(Y)

        X1 = []; #Y1 = []
        for l in range(len(X0)):
            X1.append(conv2d(np.squeeze(X0[l]), kernel, stride= (2,2), padding=(1,1)))
            #Y1.append(conv2d(Y[l], kernel, stride= (2,2), padding=(1,1)))
        X1 = np.asarray(X1)
        #Y1 = np.asarray(Y1)
        X1 = (X1-np.min(X1))/(np.max(X1)-np.min(X1))
        #Y1 = Y1/(np.max(Y1)-np.min(Y1))
        return np.expand_dims(X1,-1)
    ...
    def train_init(self, level, X0, X, Y, pred_b=None):
        ...
        if(level!=1):
            ...
            idx = np.arange(0, len(X0), 1).tolist()
            sel = random.sample(idx, int(0.5*len(idx)))
            X0 = X0[sel]
            Y = Y[sel]
        """
        N,h,w = Y.shape
        ...
        idx = random.sample(sel.tolist(), int(len(sel)/2))
        self.Train_KMeans_InitPred(X[idx], 2)
        X_K = self.Get_KMeans_InitPred(X, 2)
        """
        ...
        """
        rft = FeatureTest('rmse')
        rft.fit(X_train[sel_tr], y_train[sel_tr], n_bins=16, outliers=True)
        rft.plot(path=os.path.join(self.args['img_root'], f"train_rft_{level}_{g_name}.png"))

                #logger.info(f'Val RFT, shape: {X_val.shape}')
        rft_val = FeatureTest('rmse')
        rft_val.fit(X_val[sel_val], y_val[sel_val], n_bins=16, outliers=True)
        rft_val.plot(path=os.path.join(self.args['img_root'], f"val_rft_{level}_{g_name}.png"))

        plot_train_val_rank(rft, rft_val, path=os.path.join(self.args['img_root'], f"joint_{level}_{g_name}.png"))
        rft.n_selected = int(0.75*len(feat_idx))
        self.rft_all[f'level_init']['before_lnt_r2'] = rft
                #self.rft_feat_selection(level, feat, res,f'before_lnt_{g_name}', feat_idx, self.args['img_root'], self.args['FS'][level-1][g_ind], f'level_{level}_{g_name}')
                #feat = feat[:, self.rft_all[f'level_{level}'][f'before_lnt_{g_name}']]
        feat_tr2 = rft.transform(feat[sel_tr_all], n_selected=rft.n_selected)
        
        #feat_tr = feat[:, self.rft_all[f'level_init']['before_lnt']]
        
        if(self.args['lnt'][level-1]):
            #subsets = [0, 50, 100, 150,200,250  ]
            feat_trl = np.copy(feat_tr2)
            
            self.Train_LNT(feat_tr2, Y.reshape(-1)[sel_tr_all], 'init', g_name)
            feat_lnt = self.Get_LNT(feat_tr2, 'init', g_name)
            feat_trl = np.concatenate([feat_trl, feat_lnt], axis = -1)
            #feat_trl = feat_lnt
            #plot(feat_tr, Y.reshape(-1), feat_lnt,f'Level_{level}',  os.path.join(self.args['img_root'], 'LNT') )
            feat_idx = np.arange(0, feat_trl.shape[-1], 1)
            ...
            X_train, X_val, y_train, y_val = train_test_split(feat_trl, Y.reshape(-1)[sel_tr_all], test_size=0.2,
                                                          random_state=42)
                #RFT 
            rft = FeatureTest('rmse')
            rft.fit(X_train, y_train, n_bins=16, outliers=True)
            ...
            rft.n_selected = int(1*len(feat_idx))
            self.rft_all[f'level_init'][f'after_lnt_{g_name}'] = rft
            feat_tr2 = rft.transform(feat_trl, n_selected=rft.n_selected)
        
            #feat_tr = feat_trl[:, self.rft_all[f'level_init'][f'after_lnt_{g_name}']]
        
        self.feat_mean[f'initr2_{level}'] = np.mean(feat_tr2,axis=0,keepdims=True)
        self.feat_std[f'initr2_{level}'] = np.std(feat_tr2,axis=0,keepdims=True)
        feat_tr2 = (feat_tr2 - self.feat_mean[f'initr2_{level}'])/self.feat_std[f'initr2_{level}'] 
        print(f"Shape of features after RFT {feat_tr2.shape}")
    ...
    def get_init(self, level, X0, X, Y, mode, pred_b = None):
        ...
        abs_res = np.abs(Y.reshape(-1)-pred)
        sel = np.where(abs_res>=self.thresh['init'])
        rft = self.rft_all[f'level_init']['before_lnt_r2']
        feat_tr2 = rft.transform(feat[sel], n_selected = rft.n_selected)
        if(self.args['lnt'][level-1]):
        ...
        #pred = np.zeros(len(feat_tr2))
        rounds=int(len(feat_tr2)/1000000)+1
        respred = []
        for r in range(rounds):
            respred.extend(self.xgb_all[f'initlevel_{level}_{g_name}'][0].predict(feat_tr2[r*1000000:(r+1)*1000000]))

        pred[sel] = respred

        mse_te = calculate_mse(np.asarray(respred), Y.reshape(-1)[sel])

        if(mode == 'te'):
            print(f'Iter {"sel"}: Test MSE = {mse_te}')
        elif(mode == 'tr'):
            print(f'Iter {"sel"}: Train MSE = {mse_te}')
        
        mse_te = calculate_mse(pred, Y.reshape(-1))

        if(mode == 'te'):
            print(f'Iter {1}: Test MSE = {mse_te}')
        elif(mode == 'tr'):
            print(f'Iter {1}: Train MSE = {mse_te}')
        """
        feat_tr = None
        gc.collect()
    ...
    def load_patches(self,mode, aug, level, bias_term = None):
        ...
        for ind in range(num):
            ...
            if(aug == True):
                ...
                #patch_r=cv2.resize(H_aug, dim, interpolation=cv2.INTER_LANCZOS4)
                YUV = cv2.cvtColor(H_aug, cv2.COLOR_RGB2YUV)[:,:,0]
                YUV = cv2.Sobel(src=YUV, ddepth=cv2.CV_64F, dx=1, dy=1, ksize=5)
                YUV = minmax_normalize(YUV)
                L.extend(im_to_blocks_te(patch_size, np.expand_dims(YUV,-1)))
                patches_r = im_to_blocks_padte(patch_size, H_aug,0)
    ...
    def load_patches_te(self,mode, aug, level, bias_term = None):
        ...
        for ind in range(num):
            ...
            if(aug == True):
                ...
                Y.extend(im_to_blocks_gtte(patch_size, gt)[0])
                """
    ...
    def Get_PixelHopFeat(self, level, data_all,mode, channel, win, g_name,ud_idx = None, rot_idx= None):
        ...
        #p5.load(modelroot+'Saab_HP' + str(level)+ str(channel) + str(shrinkArgs['win']))
        if(mode == 'inp'):
    ...
    def Add_SpatialFeat(self, feat, patches, level, win, mode, ud_idx = None, rot_idx = None):
        ...
        if(mode == 'inp'):
            ...
            if(ud_idx is not None):
                ...
                """
                if(level ==4):
                    patches_laws = np.squeeze(view_as_windows(np.squeeze(patches_laws), (1,win,win),(1,1,1)))
                else: 
                    patches_laws = np.squeeze(view_as_windows(np.squeeze(patches_laws), (1,win,win),(1,2,2)))
                
                feat_filt = []
                for m in range(len(laws)):
                    for n in range(len(laws)):
                        filt = np.outer(laws[m],laws[n])
                        feat_filt.append(((patches_laws[:,:]*filt)).reshape(len(patches_laws),-1))

                feat_laws = np.column_stack(feat_filt)
                feat_laws = feat_laws.reshape(N,int(h/ld),int(w/ld),feat_laws.shape[-1])
                if(level == 4):
            ...
            if(ud_idx is not None):
                ...
                patches_laws = utils.Shrink_patch(patches, 1, win+2, 1, int((win+2)//2))
                if(level ==4):
                    patches_laws = np.squeeze(view_as_windows(np.squeeze(patches_laws), (1,win,win),(1,1,1)))
                else: 
                    patches_laws = np.squeeze(view_as_windows(np.squeeze(patches_laws), (1,win,win),(1,2,2)))
                ...
                feat_laws = np.column_stack(feat_filt)
                feat_laws = feat_laws.reshape(N,h,w,feat_laws.shape[-1])
                """
                #feat_grad = np.max(np.abs(np.gradient(feat_spa, edge_order= 2, axis = 0).reshape(len(feat_spa),-1,feat_spa.shape[-1], feat_spa.shape[-1])), axis = 1)
                #feat_grada = feat_grad.reshape(N, int(h),int(w),feat_grad.shape[-1]*feat_grad.shape[-1])
        elif(mode == 'patches'):
    ...
    def generate_lnt_features(self, X_train, y_train, comb_size, level, g_name):
        """
        Generates LNT features for all combinations of feature subsets of given size.
        Applies LNT kernel on each combination and concatenates the output features.

        Args:
            X_train: (n_train, d)
            y_train: (n_train,)
            X_test:  (n_test, d)
            comb_size: int (e.g. 2, 3, or 4)
            cls_num: int, number of classes

        Returns:
            lnt_feat_train: (n_train, num_combinations * 200)
            lnt_feat_test:  (n_test, num_combinations * 200)
            all_combs: list of feature index combinations
        """
        if(len(X_train)>500000):
            sel =random.sample(np.arange(0, len(X_train),1).tolist(), 400000)
        else: 
            sel = np.arange(0, len(X_train),1).tolist()
        all_combs = list(combinations(range(X_train.shape[1]), comb_size))
        lnt_feat_train_list = []
        self.all_combs[f'level_{level}'] = all_combs

        X_train = X_train[sel]
        y_tr = y_train.copy()
        y_trr_cls = select_comb(y_tr)
        self.lnt_all[f'level_{level}'][f'lnt_model_{g_name}'] = []
        for i, comb in enumerate(all_combs):
            X_train_sub = X_train[:, comb]
            #X_test_sub = X_test[:, comb]

            # print(f"\nGenerating LNT features for feature combination {i+1}/{len(all_combs)}: {comb}")

            cur_kernel, lnt_feat_tr = lnt_kernel(X_train_sub, y_tr[sel], y_trr_cls[sel], svd=None, num=200, if_plot=False)
            #_, lnt_feat_te = lnt_kernel(X_test_sub, y_tr, y_trr_cls, svd=cur_kernel)
            self.lnt_all[f'level_{level}'][f'lnt_model_{g_name}'].append(cur_kernel)
            lnt_feat_train_list.append(lnt_feat_tr)
            #lnt_feat_test_list.append(lnt_feat_te)

        # Concatenate features from all combinations
        lnt_feat_train = np.concatenate(lnt_feat_train_list, axis=1)
        #lnt_feat_test = np.concatenate(lnt_feat_test_list, axis=1)


        # === DFT on LNT features===
        feat_idx = np.arange(0, lnt_feat_train.shape[-1], 1)
        #_, dft_feat_loss, dft_loss_list = FEAT.feature_selection(lnt_feat_train, y_train, FStype='DFT_entropy', thrs=1.0, B=32)
        self.dft_feat_selection(level, lnt_feat_train, y_train[sel],f'lnt_{g_name}', feat_idx, self.args['img_root'], 0.4, f'level_{level}LNT_{g_name}')
        
        """
        #save_dir = './feature_check/lnt'
        #os.makedirs(save_dir, exist_ok=True)

        loss_values = dft_feat_loss
        loss_values_lnt = np.sort(loss_values)

        plt.figure(figsize=(8, 5))
        plt.plot(loss_values_lnt, color='darkgreen')
        plt.title(f'Cumulative Sorted DFT Loss - LNT features - combo_{comb_size}')
        plt.xlabel('Top Features')
        plt.ylabel('Cumulative DFT Loss (Entropy)')
        plt.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'class_{cls_num}_dft_loss_combo_{comb_size}.png'))
        plt.close()
        """
        return lnt_feat_train
    ...
    def Patch_XGBoost_Train(self, X,pred, gt, hard = None):
        ...
        """
        for aug in range(5,6):
            X_aug = self.transform_images_onetype(np.squeeze(X_sel), aug, X_sel)
            X_all.extend(np.expand_dims(X_aug[easyp],-1))
            pred_aug = self.transform_images_onetype(pred_sel16, aug, pred_sel16)
            pred_sel_all.extend(pred_aug[easyp])
            y_aug.extend([0]*len(easyp))
        """
        ...
        if(hard is None):
            ...
            """
            rounds=int(len(feat_1)/1000000)+1
            pred_cs = []; all_pred = []
            for r in range(rounds):
                all_pred.extend(self.xgb_clf1.predict(feat_1[r*1000000:(r+1)*1000000]))
                pred_cs.extend(self.xgb_clf1.predict_proba(feat_1[r*1000000:(r+1)*1000000]))
            """
            ...
            """
            entr = entropy(pred_cs_tr, base = 2, axis = 1)
            acc = []; ar = [];
            for e in np.arange(0.02, 1, 0.02):
                a = np.where(entr<=e)[0]
                ar.append(len(a)/len(entr))
                acc.append(accuracy_score(y_train[a], pred_tr[a]))
            plt.cla();plt.clf();
            plt.plot(ar, acc)
            plt.savefig("Acceptance Rate vs Accuracy")
            s = np.where(np.asarray(ar)>=0.7)[0]
            a = np.arange(0.02, 1, 0.02)
            if(self.sel_entropy is None):
                self.sel_entropy = a[s[0]]
                sel = np.where(entr>self.sel_entropy)[0]
                print(f'Selected samples; {len(sel)}')
            entr_val = entropy(pred_cs_val, base = 2, axis = 1)
            sel_val = np.where(entr_val>self.sel_entropy)[0]
            """
        else: 
            rounds=int(len(X_train)/1000000)+1
            pred_tr = []
        ...
        """
        self.dft_feat_selection('clf', feat[sel], Y_sel[sel],'bin1', feat_idx, self.args['img_root'], 0.7, f'level_clf2' )
        feat_2 = feat[:, self.dft_all['level_clf']['bin1']]
        print(f"Shape of features after RFT: {feat.shape}")
        X_train, X_val, y_train, y_val = train_test_split(feat_2[sel], Y_sel[sel], test_size=0.2,
                                                          random_state=42)
        self.xgb_clf2 = train_clf(self.args, X_train, y_train, X_val, y_val, 2, plot = True, savepath= os.path.join(args['img_root'], f'xgboost_clfbin2.png'))
        rounds=int(len(X_train)/1000000)+1
        pred_tr = []
        ...
        for r in range(rounds):
            pred_val.extend(self.xgb_clf2.predict(X_val[r*1000000:(r+1)*1000000]))
        #pred_val = self.xgb_clf.predict(X_val)
        rounds=int(len(feat)/1000000)+1
        pred_cs = []
        for r in range(rounds):
            pred_cs.extend(self.xgb_clf2.predict_proba(feat_2[r*1000000:(r+1)*1000000]))

        acc_tr = accuracy_score(y_train, pred_tr)
        acc_val = accuracy_score(y_val, pred_val)
        print(f'Classifier Train Accuracy : {acc_tr}, Val Accuracy : {acc_val}')
        """
        if(hard is None):
    ...
    def Patch_XGBoost_Pred(self, X,pred, gt, mode, hard = None):
        ...
        if(hard is None):
            pred_cs = []
            for r in range(rounds):
                pred_cs.extend(self.xgb_clf1.predict_proba(feat_rft[r*1000000:(r+1)*1000000]))
            pred_cs = np.asarray(pred_cs)
            entr = entropy(pred_cs, base = 2, axis = 1)

            acc_te = accuracy_score(gt.reshape(-1), p)
            if(mode == 'te'):
                print(f'Test Classifier Accuracy : {acc_te}')
            else:
                print(f'Train Classifier Accuracy : {acc_te}')

            
            
            
            entr = entropy(pred_cs, base = 2, axis = 1)
            sel = np.where(entr>0.9)[0]
            p[sel] = 0
            if(mode == 'te'):
                sel = np.where(entr>self.sel_entropy)[0]
            else: 
                sel = np.where(entr>self.sel_entropy)[0]
            print("Selected samples in round 1")
            acc_te = accuracy_score(gt.reshape(-1)[sel], p[sel])
            ...
            pred_cs = []
            for r in range(rounds):
                pred_cs.extend(self.xgb_clf2.predict_proba(feat_rft[r*1000000:(r+1)*1000000]))
            pred_cs = np.asarray(pred_cs)
            entr = entropy(pred_cs, base = 2, axis = 1)
            a = np.where(entr<0.6)[0]
            #p[hard[a]] = 0
            acc_te = accuracy_score(gt.reshape(-1)[hard], p[hard])
            if(mode == 'te'):
                print(f'Test Classifier Accuracy : {acc_te}')
            else:
                print(f'Train Classifier Accuracy : {acc_te}')
        ...
        feat_2 = feat[:, self.dft_all['level_clf']['bin1']][sel]
        rounds=int(len(feat_2)/1000000)+1
        xgb_pred = []
        for r in range(rounds):
            xgb_pred.extend(self.xgb_clf2.predict(feat_2[r*1000000:(r+1)*1000000]))
        p[sel] = xgb_pred
        """
    ...
    def Bin_XGBoost_Pred(self, X,pred, gt, mode, hard = None):
        level = 'clf'
        p = pred.reshape(-1).copy()
        a = np.where(p>0.4)[0]
        b = np.where(p<0.6)[0]
        ...
        p[sel] = xgb_pred
        """
        acc_te = accuracy_score(gt.reshape(-1)[c], p[c])
        rec_te = recall_score(gt.reshape(-1)[c], p[c])
        prec_te = precision_score(gt.reshape(-1)[c], p[c])
        if(mode == 'te'):
            print(f'Test Classifier Accuracy : {acc_te}, {prec_te}, {rec_te}')
        else:
            print(f'Train Classifier Accuracy : {acc_te}, {prec_te}, {rec_te}')
    ...
    """
    def Bin_XGBoost_Train(self, X, gt):
        
        idx = np.arange(0, len(X), 1)
        sel = random.sample(idx.tolist(), int(0.25*len(X)))
        X_sel = X[sel]; Y_sel = gt[sel]
        idx = np.arange(0, len(X_sel.reshape(-1)), 1)
        self.pixelhop_prev['prevlevel_clf'] = dict()
        self.Train_PixelHop('clf', np.expand_dims(X_sel, -1), np.expand_dims(X_sel, -1),np.expand_dims(X_sel, -1),'prev', 4, 7,idx , 'bin0' )
        X_tr = self.Get_PixelHopFeat('clf', np.expand_dims(X_sel, -1), 'prev', 0, 7 , idx, 'bin0')
        #X_tr = []
        X_tr = self.Add_SpatialFeat(X_tr, np.expand_dims(X_sel, -1), 'clf', 7, 'prev')
        feat_idx = np.arange(0, X_tr.shape[-1], 1)
        X_tr = X_tr.reshape(-1, X_tr.shape[-1]); Y_sel = Y_sel.reshape(-1)
        self.rft_all['level_clf'] = dict()
        self.rft_feat_selection('clf', X_tr, Y_sel,'bin', feat_idx, self.args['img_root'], 0.8, f'level_clf' )
        X_tr = X_tr[:, self.rft_all['level_clf']['bin']]
        X_train, X_val, y_train, y_val = train_test_split(X_tr, Y_sel, test_size=0.2,
                                                          random_state=42)
        self.xgb_clf = train_clf(self.args, X_train, y_train, X_val, y_val, plot = False, savepath= os.path.join(args['img_root'], f'xgboost_clf.png'))
        rounds=int(len(X_train)/1000000)+1
        pred_tr = []
        for r in range(rounds):
            pred_tr.extend(self.xgb_clf.predict(X_train[r*1000000:(r+1)*1000000]))
        #pred_tr = self.xgb_clf.predict(X_train)
        rounds=int(len(X_val)/1000000)+1
        pred_val = []
        for r in range(rounds):
            pred_val.extend(self.xgb_clf.predict(X_val[r*1000000:(r+1)*1000000]))
        #pred_val = self.xgb_clf.predict(X_val)
    ...
    def Bin_XGBoost_Pred(self, X,gt, mode):
        idx = np.arange(0, len(X.reshape(-1)), 1)
        feat = self.Get_PixelHopFeat('clf',  np.expand_dims(X, -1), 'prev', 0,5, idx, 'bin0')
        #feat = []
        feat = self.Add_SpatialFeat(feat, np.expand_dims(X, -1), 'clf', 7, 'prev' )
        feat = feat.reshape(-1, feat.shape[-1])
        feat = feat[:, self.rft_all['level_clf']['bin']]
        rounds=int(len(feat)/1000000)+1
        pred = []
        for r in range(rounds):
            pred.extend(self.xgb_clf.predict(feat[r*1000000:(r+1)*1000000]))
        #pred = self.xgb_clf.predict(feat.reshape(-1, feat.shape[-1]))
        acc_te = accuracy_score(gt.reshape(-1), pred)
        if(mode == 'te'):
            print(f'Test Classifier Accuracy : {acc_te}')
        else:
            print(f'Train Classifier Accuracy : {acc_te}')
    ...
    def homology(self, Y):
        nuc_count= []
        t = np.arange(0.2, 0.8, 0.02)
        for thresh in t: 

            pred_tr = binarize(Y, thresh).astype('uint8')
            count = 0
            fin_te, gt_arr = rec_img(Y, pred_tr.reshape(len(X_test0),args['patch_size'], args['patch_size'] ), args['patch_size'], len(X_test0), 14)

            for n in range(len(fin_te)):

                num_labels, labels, stats, centroids = cv2.connectedComponentsWithStats(fin_te[n])
                count+= len(centroids)
            nuc_count.append(count)
            #kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
            #eroded = cv2.erode(pred_tr, kernel, iterations=3)

            
        print(" ")
    def instance_classifier_train(self, reg_feat, X, y):
        feat_idx = np.arange(0, reg_feat.shape[-1], 1)
        self.rft_all[f'level_clf'] = {}
        self.lnt_all[f'level_clf'] = {}
        self.pixelhop_haar[f'clf'] = {}
        a = np.where(y==0)[0]
        b = np.where(y==1)[0]
        b_sel = random.sample(b.tolist(), int(0.5*len(b)))
        sel = np.concatenate([b_sel, a])

        self.Train_PixelHop('clf', X, np.expand_dims(X, -1),np.expand_dims(X, -1),'patches', 4, 5,idx , 'bin0', None )
        X_tr = self.Get_PixelHopFeat('clf', X, 'patches', 0, 5, 'bin0')
        reg_feat = np.concatenate([reg_feat, X_tr], axis = -1)
        self.dft_feat_selection('clf', reg_feat, y,'bin', feat_idx, self.args['img_root'], 0.75, f'level_clf1' )
        feat_1 = reg_feat[:, self.rft_all[f'level_clf']['bin']]

        if(True):
            
            self.Train_LNT(feat_1, y, 'clf', 'bin0')
            feat_lnt = self.Get_LNT(feat_1, 'clf', 'bin0')
            #feat_tr = feat_lnt.copy()
            feat_tr = np.concatenate([feat_1, feat_lnt], axis = -1)
            self.dft_feat_selection('clf', feat_tr, y,'bin2', feat_idx, self.args['img_root'], 0.75, f'level_clf1' )
            feat_tr = feat_tr[:, self.rft_all[f'level_clf']['bin2']]
        X_train, X_val, y_train, y_val = train_test_split(feat_tr[sel], y[sel], test_size=0.2,
                                                          random_state=42)

        
        scale_pos_weight = (y_train == 0).sum() / (y_train == 1).sum()
        print(f'pos weight: {scale_pos_weight}')
        self.xgb_clf2 = train_clf_inst(self.args, X_train, y_train, X_val, y_val, 1, pos_weight= scale_pos_weight, plot = True, savepath= os.path.join(args['img_root'], f'xgboost_iclfbin.png'))
        pred_tr = self.xgb_clf2.predict(X_train)
        pred_val = self.xgb_clf2.predict(X_val)

        print(f"Train Accuracy: {accuracy_score(y_train, pred_tr)}, Val Accuracy: {accuracy_score(y_val, pred_val)}")

    def instance_classifier_test(self, reg_feat, X, y):
        feat_tr = self.Get_PixelHopFeat('clf', X,  'patches', 0,5, 'bin0')
        reg_feat = np.concatenate([reg_feat, feat_tr], axis = -1)
        feat_1 = reg_feat[:, self.rft_all[f'level_clf']['bin']]
        if(True):
            ...
            feat_tr = np.concatenate([feat_1, feat_lnt], axis = -1)
            feat_tr = feat_tr[:, self.rft_all[f'level_clf']['bin2']]
        pred_prob = self.xgb_clf2.predict_proba(feat_tr)
        pred_te = self.xgb_clf2.predict(feat_tr)
        #pred_te = np.where(pred_prob[:,1]>0.6, 1, 0)
        print(f"Test Accuracy: {accuracy_score(y, pred_te)}, Test Precision: {precision_score(y, pred_te)}, Test Recall:  {recall_score(y, pred_te)}")

        return pred_te
    def watershed(self, result, image):
        fin_result=[]
        
        for n in range(len(result)):
            kernel = np.ones((3,3),np.uint8)
            markers = measure.label(morphology.erosion(result[n], morphology.disk(2)))
            distance = ndi.distance_transform_edt(result[n])
            coords = peak_local_max(distance, labels=result[n], min_distance=5)

            dots = np.zeros_like(result[n], dtype=np.uint8)
            for r, c in coords:
                dots[r, c] = 1
            #markers = measure.label(result[n], connectivity=2)
            #markers = markers +1 
            labels = watershed(image[n], dots, connectivity = 2, mask=result[n])   
            #labels = cv2.watershed(images[n], markers)
            labels = np.where(labels!=0, 1, 0)
            fin_result.append(labels)
        return np.asarray(fin_result)

    #@njit
    def load_prevlevel(self, level, mode):
        X0, Y = model.load_patches_te(mode, False, 4)

        if(level == 3):
            pred = self.get_init(4, X0,X0, Y,  'tr')
        else: 
            pred = self.get_init(4, X0,X0, Y,  'tr')
    ...
    def load_prevlevelcpm(self, level, mode):
        X0, Y = model.load_patches_tecpm(mode, False, 4)
        ...
        if(level == 3):
            ...
            for l in range(3, 0, -1): 
                X_l, Y = model.load_patches_tecpm(mode, False, l)
                pred_te = []
                if(l==1):
                    pred_te.append(model.test_level(l, X_l[:100], X_l[:100], Y[:100],  'tr' ,pred[:100]))
                    pred_te.append(model.test_level(l, X_l[100:], X_l[100:], Y[100:],  'tr' ,pred[100:]))
                    pred_te = np.concatenate([pred_te[0], pred_te[1]], axis = 0)
                    return pred_te
                else: 
                    pred = model.test_level(l, X_l, X_l, Y,  'tr' ,pred)
    ...
    def load_prevleveltnbc(self, level, mode):
        X0, Y = model.load_patches_tetnbc(mode, False, 4)
        ...
        if(level == 3):
            ...
            pred = self.get_init(4, X0,X0, Y,  'tr')
            for l in range(3, 0, -1): 
                X_l, Y = model.load_patches_tetnbc(mode, False, l)
                pred_te = []
                if(l==1):
                    pred_te.append(model.test_level(l, X_l[:100], X_l[:100], Y[:100],  'tr' ,pred[:100]))
                    pred_te.append(model.test_level(l, X_l[100:], X_l[100:], Y[100:],  'tr' ,pred[100:]))
    ...
    def load_prevlevelc(self, level, mode):
        X0, Y = model.load_patches_tec(mode, False, 4)
        ...
        if(level == 3):
            ...
            pred = self.get_init(4, X0,X0, Y,  'tr')
            for l in range(3, 0, -1): 
                X_l, Y = model.load_patches_tec(mode, False, l)
                pred_te = []
                if(l==1):
                    pred_te.append(model.test_level(l, X_l[:100], X_l[:100], Y[:100],  'tr' ,pred[:100]))
                    pred_te.append(model.test_level(l, X_l[100:], X_l[100:], Y[100:],  'tr' ,pred[100:]))
        ...
        return pred
    def load_currlevel(self, level, X, X0, Y, pred_trl):
        pred = model.test_level(level, X0, X, Y,  'tr' ,pred_trl)

        return pred
def postProc(pred, b_pred):
    N = len(pred)
    fin_pred = []
    for im in range(N):
        p = view_as_blocks(pred[im], (200,200))
        b = view_as_blocks(b_pred[im], (200,200))
        p = p.reshape(-1, 200,200)
        b = b.reshape(-1, 200,200)
        for j in range(len(p)):
            plt.cla();plt.clf();
            n, bins, _ = plt.hist(p[j].reshape(-1), bins = 100)
            bin_centers = (bins[:-1] + bins[1:]) / 2
            #Compute weighted average of bins
            t = np.sum(bin_centers * n) / np.sum(n)
            plt.savefig("Local Patch Hist")
            
            pred_ab = np.where(p[j]>t, 1, 0)
            pred_pp = np.maximum(b[j], pred_ab)
            fin_pred.append(pred_pp)

    return fin_pred

def train(model):
    if(model.K_means == True):
        X_train0, Y_train, bias_0 = model.load_patches("tr", False, 0)
        pred_tr, _ = model.train_level(0, X_train0,X_train0, Y_train)
        model.save_level(0,model.args['guslmodel'] )
    for l in range(4,0,-1):
        model_paths = os.path.join(model.args['guslmodel'], f'level_{l}.pkl')
        if(os.path.exists(model_paths)):
            model.load_level(model_paths, l)
            print(f'Level {l} loaded successfully')
        else:
            print(f'Start training level {l}')
            
            X_train, Y_train, bias = model.load_patches("tr", True, l)
            if(l ==4):
                prev_pred = model.test_level(0, X_train0,X_train0, Y_train,  'tr')
                pred_tr, _ = model.train_level(l, X_train0, X_train, Y_train, prev_pred)
            else:
                pred_tr1 = model.test_level(l, X_train0[:240], X_train[:240], Y_train[:240],  'tr',model.pred_tr[f'level_{l+1}'][:240])
                pred_tr2 = model.test_level(l, X_train0[240:], X_train[240:], Y_train[240:],  'tr',model.pred_tr[f'level_{l+1}'][240:])
                pred_prev = np.concatenate([pred_tr1, pred_tr2], axis = 0)
                pred_tr, _ = model.train_level(l, X_train0, X_train, Y_train, model.pred_tr[f'level_{l+1}'] )
...
def infer(model):
    for l in range(3,-1,-1):
        ...
        if(os.path.exists(model_paths)):
            model.load_level(model_paths, l)
            print(f'Level {l} loaded successfully')
    X_test3, Y_test,X_test0,_ = model.load_patches_te("te", False, 3)
    X_test3 = np.expand_dims(X_test3,0)
    Y_test = np.expand_dims(Y_test,0)
    X_test0 = np.expand_dims(X_test0,0)
    pred_trl = model.load_prevlevel(1, X_test3, X_test0, Y_test)
    l = 1
    X_test = X_test3[:,(3-l+1): X_test3.shape[2] - (3-l+1), (3-l+1): X_test3.shape[2] - (3-l+1) ]
    
    pred_trc = model.load_currlevel(1,X_test,X_test0, Y_test, pred_trl)
    model.xgb_clf1 = load_model( os.path.join(model.args['guslmodel'], f"XGBCLF1.pkl"))

    pred_te = model.Bin_XGBoost_Pred(X_test0, pred_trc, Y_test, 'te')
    #image=recover_img_patcho(np.squeeze(pred_te), pred_te.shape[1], img_size=1000, stride = pred_te.shape[1]-8,isRGB=False)
...
if __name__ == "__main__":
    ...
    args = {'patch_size' : 256,
            ...
            'modelroot' : '/home/cc98905/NS_GUSL/Models53/',
            'img_root' : '/home/cc98905/NS_GUSL/Img53/', 
            'guslmodel' : '/home/cc98905/NS_GUSL/GUSL_Models/GUSL_Models513/',
            'modelrootb' : '/home/cc98905/NS_GUSL/ModelsE1/',
            'img_rootb' : '/home/cc98905/NS_GUSL/ImgE1/', 
            'guslmodelb' : '/home/cc98905/NS_GUSL/GUSL_ModelsE1/',
            'model_paths' : [],
            'K_means' : False, 
    ...
    print(end-start)
    #print(emissions)
    if(args['K_means']== True):
        
        X_traina, Y_train, bias_0 = model.load_patches("tra", True, 0)
        pred_tr, _ = model.train_level(0, X_traina,X_traina, Y_train)
        X_train0, Y_train , _ = model.load_patches("tr", False, 0, bias_0)
        model.pred_tr['level_0'] = model.test_level(0, X_train0,X_train0, Y_train,  'tr')
        X_test0, Y_test, _= model.load_patches("te", False, 0, bias_0)
        model.pred_te['level_0'] = model.test_level(0, X_test0, X_test0, Y_test,  'te')
        model.save_level(0,args['guslmodel'] )

    else: 
        model_paths = os.path.join(args['guslmodel'], f'level_{0}.pkl')
        if(os.path.exists(model_paths)):
            model.load_level(model_paths, 0)
            print(f'Level {0} loaded successfully')
        else: 
            #model_paths = os.path.join(args['guslmodelb'], f'level_{0}.pkl')
            #modelb.load_level(model_paths,0)
            X_traina, Y_train = model.load_patches_te("tra", False, 4)
            #pred_tr = modelb.get_init(4, X_traina,X_traina, Y_train,  'tr')
            model.train_init(4, X_traina,X_traina, Y_train)
            X_train3, Y_train= model.load_patches_te("tr", False, 4)
            #pred_tr = modelb.get_init(4, X_train0,X_train0, Y_train,  'tr')
            model.pred_tr['level_0'] = model.get_init(4, X_train3,X_train3, Y_train,  'tr')
            X_test, Y_test = model.load_patches_te("te", False, 4)
            #pred_te = modelb.get_init(4, X_test0,X_test0, Y_test,  'tr')
            model.pred_te['level_0'] = model.get_init(4, X_test, X_test, Y_test,  'te')
            model.save_level(0,args['guslmodel'] )
    ...
    for l in range(3,1,-1):
        ...
        if(os.path.exists(model_paths)):
            ...
            """
            if(l == 4):
                model_paths = os.path.join(args['guslmodelb'], f'level_{l}.pkl')
                modelb.load_level(model_paths,l)
            else: 
                model_paths = os.path.join(args['guslmodelb'], f'level_{l}.pkl')
            ...
            #X_traina, Y_traina, bias_0 = model.load_patches_te("tra", True, 0)
            #X_train0, Y_train , _ = model.load_patches_te("tr", True, 0, bias_0)
            #X_test0, Y_test, _= model.load_patches_te("te", False, 0, bias_0)
            #X_train0, Y_train0 = model.load_patches_te("tra", False, 4)
            X_train3, Y_train = model.load_patches_te("tra", False, l)
            
            if(l == 3):
                
                
                #X_train = X_train3[:,(3-l+1): X_train3.shape[2] - (3-l+1), (3-l+1): X_train3.shape[2] - (3-l+1) ]
                #predb_tr0 = modelb.get_init(4, X_traina,X_traina, Y_traina,  'tr')
                #pred_tr = model.get_init(4, X_train0,X_train0, Y_train,  'tr')
                pred_tr = model.load_prevlevel(l, 'tr')
                #predb_tr = modelb.test_level_cs(l, X_traina, X_train, Y_train,  'tr' ,predb_tr0)
                _ = model.train_level(l, X_train3, X_train3, Y_train, pred_tr)
                #X_train, Y_train,X_train0, _ = model.load_patches_te("tr", True, l, bias)
                #model.pred_tr[f'level_{l}'] = model.test_level_cs(l, X_train0, X_train, Y_train,  'tr' ,pred_tr)
                X_test3, Y_test = model.load_patches_te("te", False, l)
                #X_test = X_test3[:,(3-l+1): X_test3.shape[2] -(3-l+1), (3-l+1): X_test3.shape[2] - (3-l+1) ]
                model.pred_te[f'level_{l}'] = model.test_level(l,X_test3, X_test3, Y_test, 'te', model.pred_te[f'level_{0}'])
            else:
                #X_train = X_train3[:,(3-l+1): X_train3.shape[2] - (3-l+1), (3-l+1): X_train3.shape[2] - (3-l+1) ]
                if(l == 4):
                    X_train, Y_train,X_train0= model.load_patches_te("tra", True, 4)
                    pred_tr = model.load_prevlevel(l, X_train, X_train0, Y_train)
                    _ = model.train_level(l, X_train0, X_train, Y_train, pred_tr)
                elif(l == 1):
                    
                    #X_train, Y_train,X_train0,_ = model.load_patches_te("tra", True, l)
                    pred_tr = model.load_prevlevel(l,'tr')
                    _ = model.train_level(l, X_train3, X_train3, Y_train, pred_tr)
                else: 
                    #X_train, Y_train,X_train0,_ = model.load_patches_te("tra", True, l)
                    ...
                    _ = model.train_level(l, X_train3, X_train3, Y_train, pred_tr)
                    """
                    X_train, Y_train, _  = model.load_patches("tr", False, l, bias)
                    pred_tr1 = model.test_level_cs(l, X_train0[:240], X_train[:240], Y_train[:240],  'tr',model.pred_tr[f'level_{l+1}'][:240])
                    pred_tr2 = model.test_level_cs(l, X_train0[240:], X_train[240:], Y_train[240:],  'tr',model.pred_tr[f'level_{l+1}'][240:])
                    pred_tr = np.concatenate([pred_tr1, pred_tr2], axis = 0)

                    idx = np.arange(0, len(pred_tr),1)
                    val = np.setdiff1d(idx, sel_hs)
                    patch_err = []
                    Y_train_red = block_reduce(Y_train, (1, 2**(l-1), 2**(l-1)), np.mean)
                    for patch in range(len(pred_tr)):
                        patch_err.append(calculate_mse(pred_tr[patch].reshape(-1), Y_train_red[patch].reshape(-1)))
                    patch_errind = np.argsort(patch_err)[::-1]  
                    _, new_sel, _ = np.intersect1d(patch_errind, val, return_indices= True)
                    new_sel = sorted(new_sel)
                    new_sel = new_sel[:int(len(sel_hs)/2)]
                    sel_hs = sel_hs[:int(len(sel_hs)/2)].tolist()
                    sel_hs.extend(patch_errind[new_sel])
                    sel_hs = np.unique(sel_hs).tolist()
                    _ = model.train_level_cs(l, X_train0[sel_hs], X_train[sel_hs], Y_train[sel_hs], model.pred_tr[f'level_{l+1}'][sel_hs], sel = sel_hs)
                    """
                #X_train, Y_train, _  = model.load_patches_te("tr", True, l, bias)
                if(l == 4):
                    #X_test, Y_test,X_test0= model.load_patches_te("te", False, 4)
                    model.pred_te[f'level_{l}'] = model.test_level(l,X_test0, X_test, Y_test, 'te', model.pred_te[f'level_{0}'])
                
                elif(l!=1):
                    #pred_tr1 = model.test_level_cs(l, X_train0[:480], X_train[:480], Y_train[:480],  'tr',model.pred_tr[f'level_{l+1}'][:480])
                    #pred_tr2 = model.test_level_cs(l, X_train0[480:], X_train[480:], Y_train[480:],  'tr',model.pred_tr[f'level_{l+1}'][480:])
                    #model.pred_tr[f'level_{l}'] = np.concatenate([pred_tr1, pred_tr2], axis = 0)
                    #mse_tr = calculate_mse(model.pred_tr[f'level_{l}'].reshape(-1), Y_train.reshape(-1))
                    X_test3, Y_test= model.load_patches_te("te", False, l)
                    #X_test = X_test3[:,(3-l+1): X_test3.shape[2] -(3-l+1), (3-l+1): X_test3.shape[2] - (3-l+1) ]
                    model.pred_te[f'level_{l}'] = model.test_level(l,X_test3, X_test3, Y_test, 'te', model.pred_te[f'level_{l+1}'])
                    #mse_te = calculate_mse(model.pred_te[f'level_{l}'].reshape(-1), Y_test.reshape(-1))
                    #print(f'Level {l} MSE Train : {mse_tr}, MSE Test : {mse_te}')
                
                else: 
                    #model.pred_tr[f'level_{l}'] = model.test_level_haar(l, X_train0, X_train, Y_train,  'tr',model.pred_tr[f'level_{l+1}'])
                    #pred_tr1 = model.test_level_cs(l, X_train0[:480], X_train[:480], Y_train[:480],  'tr',model.pred_tr[f'level_{l+1}'][:480])
                    #pred_tr2 = model.test_level_cs(l, X_train0[480:], X_train[480:], Y_train[480:],  'tr',model.pred_tr[f'level_{l+1}'][480:])
                    #model.pred_tr[f'level_{l}'] = np.concatenate([pred_tr1, pred_tr2], axis = 0)
                    #mse_tr = calculate_mse(model.pred_tr[f'level_{l}'].reshape(-1), Y_train.reshape(-1))
                    X_test3, Y_test = model.load_patches_te("te", False, 3)
                    #X_test = X_test3[:,(3-l+1): X_test3.shape[2] -(3-l+1), (3-l+1): X_test3.shape[2] - (3-l+1) ]
                    model.pred_te[f'level_{l}'] = model.test_level(l,X_test3, X_test3, Y_test, 'te', model.pred_te[f'level_{l+1}'])
                    #mse_te = calculate_mse(model.pred_te[f'level_{l}'].reshape(-1), Y_test.reshape(-1))
                    #print(f'Level {l} MSE Train : {mse_tr}, MSE Test : {mse_te}')
            ...
            Y_train_red = block_reduce(Y_train, (1, 2**(l-1), 2**(l-1)), np.mean)
            Y_test_red = block_reduce(Y_test, (1, 2**(l-1), 2**(l-1)), np.mean)
            #Y_train_red = Y_train.copy()
            #Y_test_red = Y_test.copy()
            #for l_ in range(l-1):
            #        Y_train_red = np.squeeze(model.lp(Y_train_red))
            #        Y_test_red = np.squeeze(model.lp(Y_test_red))
            if(l<3):
                pred_trl = model.load_prevlevel(l, 'tr')
                pred_tr = upscale_pred(pred_trl,(2*pred_trl.shape[-1], 2*pred_trl.shape[-1]), order= 'lanczos' )
                pred_te = upscale_pred(model.pred_te[f'level_{l+1}'],(2*model.pred_te[f'level_{l+1}'].shape[-1], 2*model.pred_te[f'level_{l+1}'].shape[-1]), order= 'lanczos' )
                #pred_tr = upsample(pred_trl,(2*pred_trl.shape[-1]))
                #pred_te = upsample(model.pred_te[f'level_{l+1}'],(2*model.pred_te[f'level_{l+1}'].shape[-1]))
                pred_tr = truncation(pred_tr)
                pred_te = truncation(pred_te)
                print(calculate_mse(pred_tr.reshape(-1), Y_train_red.reshape(-1)))
                print(calculate_mse(pred_te.reshape(-1), Y_test_red.reshape(-1)))
            if(l == 3):
                #_, Y_train, _  = model.load_patches_te("tr", False, l, bias)
                #Y_train_redp = block_reduce(Y_train, (1, 2**(l-1), 2**(l-1)), np.mean)
                pred_trl = model.load_prevlevel(l, 'tr')
                pred_tr = upscale_pred(pred_trl,(2*pred_trl.shape[-1], 2*pred_trl.shape[-1]), order= 'lanczos' )
                pred_te = upscale_pred(model.pred_te[f'level_{0}'],(2*model.pred_te[f'level_{0}'].shape[-1], 2*model.pred_te[f'level_{0}'].shape[-1]), order= 'lanczos' )
                #pred_tr = upsample(pred_trl,(2*pred_trl.shape[-1]))
                #pred_te = upsample(model.pred_te[f'level_{0}'],(2*model.pred_te[f'level_{0}'].shape[-1]))
                pred_tr = truncation(pred_tr)
                pred_te = truncation(pred_te)
                print(calculate_mse(pred_tr.reshape(-1), Y_train_red.reshape(-1)))
                print(calculate_mse(pred_te.reshape(-1), Y_test_red.reshape(-1)))
            model.pred_tr[f'level_{l}'] = model.load_currlevel(l, X_train3, X_train3, Y_train, pred_trl)
            print(calculate_mse( model.pred_tr[f'level_{l}'] .reshape(-1), Y_train_red.reshape(-1)))
            print(calculate_mse(model.pred_te[f'level_{l}'].reshape(-1), Y_test_red.reshape(-1)))
            model.save_level(l,args['guslmodel'] )
    
    #X_traina, Y_train, bias_0,_ = model.load_patches_te("tra", True, 0)
    #X_train0, Y_train , _, _ = model.load_patches_te("tr", True, 0, bias_0)
    #X_test0, Y_test, _,_= model.load_patches_te("te", False, 0, bias_0)
    l = 1
    X_train3, Y_train= model.load_patches_te("tra", False, l)
    #X_train3c, Y_trainc,X_train0c,_ = model.load_patches_tec("tr", False, 3)
    X_test3, Y_test= model.load_patches_te("te", False, l)
    X_test = X_test3[:,(3-l+1): X_test3.shape[2] -(3-l+1), (3-l+1): X_test3.shape[2] - (3-l+1) ]
    #X_test = X_test3[:, 2: X_test3.shape[2] -2, 2: X_test3.shape[2] -2]
    #X_train3cons, Y_traincons,X_train0cons,_ = model.load_patches_tecons("te", False, 3)
    X_train = X_train3[:,(3-l+1): X_train3.shape[2] - (3-l+1), (3-l+1): X_train3.shape[2] - (3-l+1) ]
    #X_trainc = X_train3c[:,(3-l+1): X_train3c.shape[2] - (3-l+1), (3-l+1): X_train3c.shape[2] -(3-l+1) ]
    #X_train = X_train3[:, 2:X_train3.shape[2] - 2,2:X_train3.shape[2] - 2]
    #X_traincons = X_train3cons[:,2*(3-l): X_train3cons.shape[2] - 2*(3-l), 2*(3-l): X_train3cons.shape[2] - 2*(3-l) ]
    idx = np.arange(0, len(X_train3), 1)
    #sel_idx = load_model(os.path.join(model.args['modelroot'], f"Sel.pkl"))
    sel_idx = random.sample(idx.tolist(), int(0.02*len(X_train3)))
    #save_model(sel_idx, os.path.join(model.args['modelroot'], f"Sel.pkl"))
    idx_val = np.setdiff1d(idx, sel_idx)
    sel_val = random.sample(idx_val.tolist(), int(0.3*len(X_train3)))
    #pred_trcons = model.load_prevlevel(1, X_train3cons, X_train0cons, Y_traincons)
    #pred_trcr = model.load_prevlevel(1, X_train3c, X_train0c, Y_trainc)
    #pred_trl = model.load_prevlevel(1, 'tr')
    #pred_vall = model.load_prevlevel(1, X_train3[sel_val], X_train0[sel_val], Y_train[sel_val])
    model_paths = os.path.join(args['guslmodel'], f'level_{l}.pkl')
    ...
    if(os.path.exists(model_paths)):
        ...
        _ = model.train_level(l, X_train3[sel_idx], X_train3[sel_idx], Y_train[sel_idx], model.pred_tr['level_2'][sel_idx], sel = sel_idx)
        #_ = model.Bin_XGBoost_Train(X_train0[sel_idx], pred_trc, Y_train[sel_idx] ) 
        #sel_hard_tr = model.Bin_XGBoost_Pred(X_train0[sel_idx], pred_trc, Y_train[sel_idx], 'tr')
        #pred_trc1 = model.train_level(l, X_train0[sel_idx], X_train[sel_idx], Y_train[sel_idx], pred_trc, sel = sel_idx, idx_hard = sel_hard_tr )
    
    
    #_ = model.train_level(l, X_train3[sel_idx], X_train3[sel_idx], Y_train[sel_idx], model.pred_tr['level_1'][sel_idx], sel = sel_idx)


    """
    rounds = int(len(X_train3)/4)+1
    model.pred_tr['level_1'] = []
    for c in range(4):
        model.pred_tr['level_1'].append(model.test_level(l,(X_train3[(c*120):((c+1)*120)]), (X_train3[(c*120):((c+1)*120)]), Y_train[(c*120):((c+1)*120)], 'tr', model.pred_tr['level_2'][(c*120):((c+1)*120)]))
    model.pred_tr['level_1'] = np.concatenate([model.pred_tr['level_1'][0], model.pred_tr['level_1'][1], model.pred_tr['level_1'][2], model.pred_tr['level_1'][3]], axis = 0)
    
    rounds = int(len(X_test3)/4)+1
    model.pred_te['level_1'] = []
    for c in range(4):
        model.pred_te['level_1'].append(model.test_level(l,(X_test3[(c*56):((c+1)*56)]), (X_test3[(c*56):((c+1)*56)]), Y_test[(c*56):((c+1)*56)], 'te', model.pred_te[f'level_{l+1}'][(c*56):((c+1)*56)]))
    model.pred_te['level_1'] = np.concatenate([model.pred_te['level_1'][0], model.pred_te['level_1'][1], model.pred_te['level_1'][2], model.pred_te['level_1'][3]], axis = 0)
    model.save_level(l,args['guslmodel'] )
    print(calculate_mse(model.pred_te[f'level_{l}'].reshape(-1), Y_test.reshape(-1)))
    """
    #gt, l1 = rec_img(model.pred_te['level_1'].reshape(len(Y_test),args['patch_size'], args['patch_size'] ), Y_test.reshape(len(Y_test),args['patch_size'], args['patch_size'] ), args['patch_size'], len(Y_test), 14)
    #gt = np.asarray(gt)
    #l1 = np.asarray(l1)
    
    """
    gt_te, l1_te = rec_img(model.pred_te['level_1'].reshape(len(Y_test),args['patch_size'], args['patch_size'] ), Y_test.reshape(len(Y_test),args['patch_size'], args['patch_size'] ), args['patch_size'], len(Y_test), 14)
    ...
    _ = model.Bin_XGBoost_Train(X_train3[sel_idx,5:266-5,5:266-5], model.pred_tr['level_1'][sel_idx], Y_train[sel_idx] ) 
    pred_te = model.Bin_XGBoost_Pred(X_test3[:,5:266-5,5:266-5], model.pred_te[f'level_{l}'], Y_test, 'te')
    #model.save_level_clf('clf',args['guslmodel'] )
    ...
    X_test3cpm, Y_testcpm= model.load_patches_tec("te", False, l)
    pred_cpm = model.load_prevlevelc(0, mode = 'te')
    #pred_te = model.Bin_XGBoost_Pred(X_test3[:,5:266-5,5:266-5], model.pred_te[f'level_{l}'], Y_test, 'te')
    pred_tecpm = model.Bin_XGBoost_Pred(X_test3cpm[:,5:266-5,5:266-5], pred_cpm, Y_testcpm, 'te')

    gt_tecpm, l1_tecpm = rec_img512(pred_tecpm.reshape(len(Y_testcpm),args['patch_size'], args['patch_size'] ), Y_testcpm.reshape(len(Y_testcpm),args['patch_size'], args['patch_size'] ), args['patch_size'], len(Y_testcpm), 30)
    gt_tecpm = np.asarray(gt_tecpm)
    l1_tecpm = np.asarray(l1_tecpm)

    l1_bcpm = binarize(l1_tecpm, 0.5).astype('uint8')
    
    fin_ajicpm = find_aji32(gt_tecpm,l1_bcpm)
    print(fin_ajicpm)
    
    

    gt_tr, l1_tr = rec_img(model.pred_tr['level_1'].reshape(len(Y_train),args['patch_size'], args['patch_size'] ), Y_train.reshape(len(Y_train),args['patch_size'], args['patch_size'] ), args['patch_size'], len(Y_train), 30)
    gt_tr = np.asarray(gt_tr)
    l1_tr = np.asarray(l1_tr)

    im_tr, Y_tr = model.load_im('tr', False, 1)
    r, feat, y,cc, X = update_pred(l1_tr, Y_tr, im_tr)
    model.instance_classifier_train(feat,X, y)

    gt_te, l1_te = rec_img(model.pred_te['level_1'].reshape(len(Y_test),args['patch_size'], args['patch_size'] ), Y_test.reshape(len(Y_test),args['patch_size'], args['patch_size'] ), args['patch_size'], len(Y_test), 14)
    gt_te = np.asarray(gt_te)
    l1_te = np.asarray(l1_te)

    im_te, Y_te = model.load_im('te', False, 1)
    r_test, feat_test, y_test, cc_test, X_te = update_pred(l1_te, Y_te, im_te)
    pred_te_inst = model.instance_classifier_test(feat_test,X_te, y_test)
    #gt_te, l1_te = rec_img(pred_te.reshape(len(Y_test),args['patch_size'], args['patch_size'] ), Y_test.reshape(len(Y_test),args['patch_size'], args['patch_size'] ), args['patch_size'], len(Y_test), 14)
    ...
    idx = 0
    mask = np.ones(cc_test.shape)
    for r in range(len(r_test)):
        val = np.max(cc_test[r])
        for i in range(len(r_test[r])):
            curr = r_test[r][i][0]
            mask[r][cc_test[r]==curr.label] = 0
            if(pred_te_inst[idx]==1):
                cc_test[r][cc_test[r]==curr.label] = val+1
            idx+=1
        cc_test[r][cc_test[r]!=val+1] = 0
        cc_test[r][cc_test[r]==val+1] = 1
    ...
    fin_aji = find_aji32(gt_te,inter)
    print(fin_aji)
    #model.pred_te[f'level_{l}']  = model.test_level(l,X_test3, X_test3, Y_test, 'te', model.pred_te[f'level_{l+1}'])
    ...
    model.pred_tr['level_1'] = []
    for c in range(4):
        model.pred_tr['level_1'].append(model.test_level(l,X_train3[(c*120):((c+1)*120)], X_train3[(c*120):((c+1)*120)], Y_train[(c*120):((c+1)*120)], 'tr', pred_trl[(c*120):((c+1)*120)]))
    model.save_level(l,args['guslmodel'] )
    print(calculate_mse(model.pred_te[f'level_{l}'].reshape(-1), Y_test.reshape(-1)))
    """
    """"""
    X_train3, Y_train= model.load_patches_te16("tra", False, l)
    X_test3, Y_test= model.load_patches_te16("te", False, l)
    _ = model.Patch_XGBoost_Train(X_train3, model.pred_tr['level_1'], Y_train) 
    pred_tr_sel = model.Patch_XGBoost_Pred(X_train3, model.pred_tr[f'level_1'], Y_train, 'tr')
    pred_te_sel = model.Patch_XGBoost_Pred(X_test3, model.pred_te[f'level_1'], Y_test, 'te')

    sel = random.sample(pred_tr_sel.tolist(), 30000)
    _ = model.train_level16(l, X_train3[sel], X_train3[sel], Y_train[sel], model.pred_tr['level_1'], sel = sel)
    pred15  = model.test_level16(l,X_test3[pred_te_sel], X_test3[pred_te_sel], Y_test[pred_te_sel], 'te', model.pred_te[f'level_{1}'], pred_te_sel)




    
    #sel_hard_te = model.Bin_XGBoost_Pred(X_test0, model.pred_te[f'level_{l}'], Y_test, 'te')
    #model.pred_te[f'level_{l}']  = model.test_level(l,X_test0, X_test, Y_test, 'te', model.pred_te[f'level_{l}'], idx_hard = sel_hard_te )
    #pred_te = binarize(model.pred_te['level_1'], 0.5)
    #fin_aji_test = find_aji(Y_test, pred_te.reshape(len(X_test0),args['patch_size'], args['patch_size'] ), args['patch_size'], len(X_test0), 14)
    
    #pred_trconsc = model.load_currlevel(1, X_traincons, X_train0cons, Y_traincons,pred_trcons )
    #pred_trcrc = model.load_currlevel(1, X_trainc, X_train0c, Y_trainc,pred_trcr )
    #pred_trc = model.load_currlevel(1, X_train[sel_idx], X_train0[sel_idx], Y_train[sel_idx], pred_trl)
    #pred_valc = model.load_currlevel(1, X_train[sel_val], X_train0[sel_val], Y_train[sel_val], pred_vall)
    #model.xgb_clf1 = load_model( os.path.join(model.args['modelroot'], f"XGBCLF1.pkl"))
    #model.sel_entropy = load_model(os.path.join(model.args['modelroot'], f"Entropy.pkl"))
    _ = model.Bin_XGBoost_Train(X_train0[sel_idx], pred_trc, Y_train[sel_idx] ) 
    #save_model(model.xgb_clf1, os.path.join(model.args['guslmodel'], f"XGBCLF1.pkl"))
    #save_model(model.sel_entropy, os.path.join(model.args['modelroot'], f"Entropy.pkl"))
    #pred_tr, sel_tr = model.Bin_XGBoost_Pred(X_train0[sel_val], pred_valc, Y_train[sel_val], 'tr')
    pred_te = model.Bin_XGBoost_Pred(X_test0, model.pred_te[f'level_{l}'], Y_test, 'te')
    pred_tec = model.Bin_XGBoost_Pred(X_train0c, pred_trcrc, Y_trainc, 'te')
    #pred_tecons, sel_tecons = model.Bin_XGBoost_Pred(X_train0cons, pred_trconsc, Y_traincons, 'te')
    
    #model.save_level(l,args['guslmodel'] )
    
    """
    _ = model.train_level(l, X_train0[sel_val], X_train[sel_val], Y_train[sel_val], pred_vall, sel = sel_idx, idx_hard = sel_tr)
    pred_tr_r2 = model.test_level(l,X_train0[sel_val], X_train[sel_val], Y_train[sel_val], 'tr', pred_vall, idx_hard = sel_tr)
    pred_te_r2 = model.test_level(l,X_test0, X_test, Y_test, 'te', model.pred_te[f'level_{l+1}'], idx_hard = sel_te)
    pred_valc = pred_valc.reshape(-1)
    pred_valc[sel_tr] = pred_tr_r2
    pred_valc = pred_valc.reshape(len(sel_val), X_train0.shape[2], X_train0.shape[2])
    model.pred_te[f'level_{l}'] = model.pred_te[f'level_{l}'].reshape(-1)
    model.pred_te[f'level_{l}'][sel_te] = pred_te_r2
    model.pred_te[f'level_{l}'] = model.pred_te[f'level_{l}'].reshape(len(X_test0), X_test0.shape[2], X_test0.shape[2])
    """
    final_pred_te = pred_te.copy()
    fin_pred_cr = pred_tec.copy()
    #fin_pred_cons = pred_tecons.copy()
    #model.save_level(l,args['guslmodel'] )
    #model.xgb_clf2 = load_model( os.path.join(model.args['modelroot'], f"XGBCLF2.pkl"))
    #model.Bin_XGBoost_Train(X_train0[sel_val], pred_valc, Y_train[sel_val], hard = sel_tr )
    #save_model(model.xgb_clf2, os.path.join(model.args['modelroot'], f"XGBCLF2.pkl"))
    #pred_tr = model.Bin_XGBoost_Pred(X_train0[sel_idx], pred_trc, Y_train[sel_idx], 'tr', hard = sel_tr )
    #pred_te_bin = model.Bin_XGBoost_Pred(X_test0, model.pred_te['level_1'], Y_test, 'te', hard = sel_te )
    #pred_te = pred_te.reshape(-1)
    #pred_te[sel_te] = pred_te_bin.reshape(-1)[sel_te]
    #final_pred_te = pred_te.reshape(X_test0.shape)
    #model.save_level(l,args['guslmodel'] )
    #pred_trl = model.load_prevlevel(l, X_train3, X_train0, Y_train)
    #pred_trc = model.load_currlevel(l, X_train, X_train0, Y_train, pred_trl)
    """
    model_paths = os.path.join(args['guslmodelb'], f'level_{0}.pkl')
    modelb.load_level(model_paths,0)
    predb_tr = modelb.get_init(4, X_test0,X_test0, Y_test,  'te')
    X_train, Y_train, bias = model.load_patches("tra", False, l)

    for l in range(4, 0, -1):
        X_train, Y_train, bias = model.load_patches("tra", False, l)
        X_test, Y_test, _= model.load_patches_te("te", False, l, bias)
        model_paths = os.path.join(args['guslmodelb'], f'level_{l}.pkl')
        if(os.path.exists(model_paths)):
            modelb.load_level(model_paths, l)
            predb_tr= modelb.test_level_cs(l, X_test0, X_test, Y_test,  'te' ,predb_tr)
            
            print(f'Level {l} loaded successfully')
    l =1 
    
    model.pred_te[f'level_{l}'] = model.test_level_cs(l,X_test0, X_test, Y_test, 'te', model.pred_te[f'level_{l+1}'],predb_tr)
    """
    #pred_te_b = model.select_roi_im(X_test0, np.expand_dims(model.pred_te['level_1'],-1), 1, 'te')
    #filled = binary_fill_holes(pred_te_b).astype(np.uint8) * 255
    #model.homology(model.pred_te['level_0'])
    #pred = model.watershed(model.pred_te['level_1'], 0.36)
    #pred_te = binarize(model.pred_te['level_1'], 0.36)
    fin_teb, fin_tep = rec_img(model.pred_te['level_1'].reshape(len(X_test0),args['patch_size'], args['patch_size'] ), final_pred_te.reshape(len(X_test0),args['patch_size'], args['patch_size'] ), args['patch_size'], len(X_test0), 14)
    pred_tepp = postProc(fin_tep, fin_teb)
    pred_tepp = np.asarray(pred_tepp)
    fin_tepp, _ = rec_imgo(pred_tepp.reshape(350,200,200), pred_tepp.reshape(350,200, 200 ),200, 350, 14)
    fin_teb, gt_arr = rec_img(Y_test, final_pred_te.reshape(len(X_test0),args['patch_size'], args['patch_size'] ), args['patch_size'], len(X_test0), 14)
    gt_arr = np.asarray(gt_arr)
    fin_tepp = np.asarray(fin_tepp)
    #model_paths = os.path.join(args['guslmodelb'], f'level_{1}.pkl')
    #modelb.load_level(model_paths,1)
    
    #model.Bin_XGBoost_Train(model.pred_tr['level_1'], modelb.pred_tr['level_1'], Y_train ) 
    #final_pred_tr = model.Bin_XGBoost_Pred( model.pred_tr['level_1'], modelb.pred_tr['level_1'], Y_train, 'tr')
    #final_pred_te = model.Bin_XGBoost_Pred(model.pred_te['level_1'], modelb.pred_te['level_1'], Y_test, 'te')  
    #final_pred_tr = model.Bin_XGBoost_Pred(model.pred_tr['level_1'], Y_train, 'tr')
    
    #final_pred_tr = binarize(model.pred_tr['level_1'], 0.4)
    #fin_aji_train = find_aji(Y_train, final_pred_tr.reshape(len(X_train0),args['patch_size'], args['patch_size'] ), args['patch_size'], len(X_train0), 30)
    fin_aji_test = find_aji(Y_test, final_pred_te.reshape(len(X_test0),args['patch_size'], args['patch_size'] ), args['patch_size'], len(X_test0), 14)
    fin_aji_testp = find_aji_w(gt_arr, fin_tepp, args['patch_size'], len(X_test0), 14)
    #print(f'Final AJI scores: Train = {fin_aji_train}, Test = {fin_aji_test}')
    print("AJI without Morphological Processing: ", fin_aji_test)
    fin_aji_test = find_aji_m(Y_test, final_pred_te.reshape(len(X_test0),args['patch_size'], args['patch_size'] ), args['patch_size'], len(X_test0), 14)
    print("AJI with Morphological Processing: ", fin_aji_test)
    print("AJI with Post Processing: ", fin_aji_testp)
    fin_aji_test = find_aji(Y_trainc, fin_pred_cr.reshape(len(X_train0c),args['patch_size'], args['patch_size'] ), args['patch_size'], len(X_train0c), 30)
    print("CryoNuSeg: AJI without Morphological Processing: ", fin_aji_test)
    
    
    #fin_aji_test = find_aji(Y_traincons, fin_pred_cons.reshape(len(X_train0cons),args['patch_size'], args['patch_size'] ), args['patch_size'], len(X_train0cons), 14)
    #print("CoNSeP: AJI without Morphological Processing: ", fin_aji_test)
    #fin_teb, fin_tep = rec_img(pred_trconsc.reshape(len(X_train0cons),args['patch_size'], args['patch_size'] ), fin_pred_cons.reshape(len(X_train0cons),args['patch_size'], args['patch_size'] ), args['patch_size'], len(X_train0cons), 14)
    ##pred_tepp = postProc(fin_tep, fin_teb)
    ##pred_tepp = np.asarray(pred_tepp)
    #fin_tepp, _ = rec_imgo(pred_tepp.reshape(350,200,200), pred_tepp.reshape(350,200, 200 ),200, 350, 14)
    ##fin_tepp = np.asarray(fin_tepp)
    #fin_teb, gt_arr = rec_img(Y_traincons, fin_pred_cons.reshape(len(X_test0),args['patch_size'], args['patch_size'] ), args['patch_size'], len(Y_traincons), 14)
    #gt_arr = np.asarray(gt_arr)
    #fin_aji_testp = find_aji_w(gt_arr, fin_tepp, args['patch_size'], len(X_test0), 14)
    #print("CoNSeP: AJI after Morphological Processing: ", fin_aji_testp)
    print(" ")
