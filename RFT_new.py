...
class FeatureTest:
    ...
    def fit(self, X, y, n_bins, outliers=False):
        self.dim = X.shape[1]
        if(X.shape[0]>1500000):
            idx = np.arange(0, len(X)).tolist()
            sel = random.sample(idx, 1500000)
            X = X[sel]
            y = y[sel]
    ...
    def plot(self, title="RFT Loss", label_dims=None, path=None, n_selected=None):
        ...
        if label_dims is not None:
            colors = cm.rainbow(np.linspace(0, 1, len(label_dims)))
            for i, (label, dim) in enumerate(label_dims.items()):
                rs = [self.dim_rank[d] for d in dim]
                ls = [self.dim_loss[d] for d in dim]
                plt.scatter(rs, ls, s=15, color=colors[i], label=f'{label}, {dim[-1] - dim[0] + 1}')
                plt.legend()

        if n_selected is not None:
            plt.axvline(x=n_selected, color='red')

        plt.plot(np.arange(self.dim), np.array(list(self.dim_loss.values())))
        # plt.legend()
        plt.xlabel('Rank')
