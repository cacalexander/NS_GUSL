...
from torch_SSL.torch_configs import device, convert_to_torch, convert_to_numpy
from class_data_modifier import DATA_MODIFIER
import os
...
def patch_alignment(X, ud_idx, rot_idx):
        
	input_patches_ori = X.cpu().numpy().copy()
	input_patches_ori = input_patches_ori.transpose(0,2,1)
	win = int(np.sqrt(input_patches_ori.shape[-1]))
	input_patches_ori = input_patches_ori.reshape(-1,1,win,win)
	...
	X_aug = X_aug.reshape(2*len(X), X.shape[-1],-1)
	X_aug = X_aug.transpose(0,2,1)
	return convert_to_torch(X_aug).cuda()
...
class torchSaab(nn.Module):
	def __init__(self, kernel_size, stride,pad,ud_idx, rot_idx,grp, channelwise):
		super().__init__()
		...
		self.grp = np.asarray([list(range(x * 9, (x + 1) * 9)) for x in grp]).reshape(-1)
	...
	def fit(self, x, bias=None):
		...
		n = x.size(0)
		sample_patches = F.unfold(x, self.kernel_size, stride=self.stride, padding= self.pad)
		#sample_patches = sample_patches.unfold(2,self.kernel_size[0], self.stride[0])
		sample_patches = sample_patches.permute(0,2,1)
		sample_patches = sample_patches.reshape(-1,self.kernel_size[0], self.kernel_size[1])
		win = self.kernel_size[0] - 2
		pad = 0
		"""
		if(self.pad ==(0,0)):
			pad = 2*int(self.kernel_size[0]//2)
		else: 
			pad = 0
		"""
		sample_patches_all = F.unfold(sample_patches, win, stride=self.stride, padding= pad)
		sample_patches_ = sample_patches_all[self.grp]
		self.w = (self.in_w-self.kernel_size[0] + 2*self.pad[0])//self.stride[0]+1
		self.h = (self.in_h-self.kernel_size[1] + 2*self.pad[1])//self.stride[1]+1
		#sample_patches = F.unfold(x, self.kernel_size, stride=self.stride, padding= self.pad) #<n, dim, windows>
		...
		dim = sample_patches_.shape[-1]
		rank_ac = dim-1
		# Only selected samples
		sample_patches_ac, dc = remove_mean(sample_patches_, dim=1)  # Remove patch mean
		...
		U,s,V = torch.linalg.svd(torch.nan_to_num(training_data), full_matrices=False)
		...
		#All samples 
		sample_patches_ac, dc = remove_mean(sample_patches_all, dim=1)  # Remove patch mean
		training_data, feature_expectation = remove_mean(sample_patches_ac, dim=0)
		dc = dc * num_channels * 1.0 / np.sqrt(num_channels)
		ac = torch.matmul(training_data, torch.transpose(self.ac_kernels, 1,0))
		transformed = torch.cat([dc, ac], dim=1)
	...
	def saab(self, x, mode):
		...
		sample_patches = F.unfold(x, self.kernel_size, stride=self.stride, padding= self.pad)
		if(mode == 'tr' and self.ud_idx is not None):
			n = x.size(0)*2
			sample_patches = patch_alignment(sample_patches, self.ud_idx, self.rot_idx)
			sample_patches = sample_patches.permute(0,2,1)
			sample_patches = sample_patches.reshape(-1,self.kernel_size[0], self.kernel_size[1])
		else: 
			n = x.size(0)
		...
		if(self.pad ==(0,0)):
			...
			pad = 0
		"""
		sample_patches_ = F.unfold(sample_patches, win, stride=self.stride, padding= pad)
		
		#self.w = (self.in_w-self.kernel_size[0] + 2*self.pad[0])//self.stride[0]+1
		#self.h = (self.in_h-self.kernel_size[1] + 2*self.pad[1])//self.stride[1]+1 #<n, dim, windows>
		#n, dim, windows = sample_patches.size()
		#sample_patches = sample_patches.permute(0,2,1)#<n, windows, dim>
		#sample_patches = sample_patches.reshape(n*windows, dim)#<n.windows, dim>
		sample_patches_ac, dc = remove_mean(sample_patches_, dim=1)  # Remove patch mean
...
if __name__=="__main__":
	...
	grp = random.sample(idx.tolist(), 5000)
	ud_idx = idx.copy()
	rot_idx = idx.copy()
	layer1 = torchSaab(kernel_size=(5,5), stride=(1,1), pad = (2,2), ud_idx= ud_idx, rot_idx = rot_idx, grp = grp, channelwise=False)
